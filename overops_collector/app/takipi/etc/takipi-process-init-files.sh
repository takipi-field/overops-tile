...
function main()
{
	local overops_max_file_descriptors=""
	local overops_user_name="root"
	local template_files_dir="$utils_script_dir"
	local pid_file_path="/var/run/takipi.pid"
	local output_dir="$utils_script_dir/init"
	
	mkdir -p "$output_dir"
	
	local os_init_system_type="systemv"
	local os_distribution_name="Ubuntu"
	
	if ! overops_prepare_init_file "$output_dir/systemv.$os_distribution_name.takipi" "$template_files_dir" "$pid_file_path"; then
		log $error "Error preparing $os_distribution_name with $os_init_system_type"
		return 1
	fi
	
	local os_distribution_name="Redhat"
	...
	local os_distribution_name="Gentoo"
	...
	local os_distribution_name="SuSE"
	...
	local os_init_system_type="upstart"
	local os_distribution_name="Ubuntu"
	
	if ! overops_prepare_init_file "$output_dir/upstart.$os_distribution_name.takipi" "$template_files_dir" "$pid_file_path"; then
		log $error "Error preparing $os_distribution_name with $os_init_system_type"
		return 1
	fi
	...
	local os_init_system_type="systemd"
	
	if ! overops_prepare_init_file "$output_dir/systemd.service" "$template_files_dir" "$pid_file_path"; then
		log $error "Error preparing $os_distribution_name with $os_init_system_type"
		return 1
	fi
